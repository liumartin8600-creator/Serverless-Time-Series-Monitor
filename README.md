...
### 1. `index_new.js` 代码结构分区整理

- 对 `index_new.js` 按职责增加清晰的代码分区标识，提升后续阅读、定位和维护效率。
...
- 将宏观缓存写入冷却时间戳等运行态变量集中放置，方便排查缓存刷新和定时任务执行状态。
- 保持变量初始化值、作用域和调用行为不变。

### 3. 高风险区域保持原位

- 前端 HTML / CSS / JavaScript 大型模板字符串保持原有结构，不进行拆分或改写。
- API 路由匹配顺序保持不变，避免影响鉴权、接口返回和页面访问。
- `scheduled` / cron、`export default`、实时大屏、今日雷达、估值修正、AI 分析和 Webhook 推送等入口逻辑保持不变。

### 4. 可维护性与安全检查

- 完成结构整理后进行了语法检查，确认文件可被正常解析。
- 检查重复声明和关键全局变量位置，避免整理过程中引入未定义变量或初始化顺序问题。
- 本次调整仅用于代码可读性和维护性提升，不改变页面显示效果、接口行为、数据结构、业务计算结果或定时任务逻辑。

## V3.8.1 更新说明

### 1. 估值修正逻辑检查与修复

- 检查 Tab“估值修正”中系统测算误差偏大的原因，梳理真实净值、估算净值、收盘价、真实溢价、估算溢价与系统误差之间的完整计算链路。
- 检查同一行数据中收盘价、实际净值、估算净值是否存在交易日口径不一致的问题，并优化估值修正 Tab 的取数口径，尽量保证同一行数据使用同一交易日口径。
- 修正估算溢价与估算净值可能来自不同来源的问题，确保估算溢价、真实溢价和系统误差的计算定义保持一致。
- 检查盘中估值、盘后估值、历史估值混用风险，降低估值口径混乱导致的异常误差。

### 2. 估值链路时间口径检查

- 检查估算净值、Spread 定时点、`AGENT_SNAPSHOT_V1`、`Est_NAV` 与 `radar_history` 等缓存或历史数据的写入时间和读取优先级。
- 检查北京时间、UTC 时间和交易日日期之间可能出现的错位问题。
- 优化估值相关数据在盘中、盘后、历史回放中的匹配逻辑，降低时间错位导致的估值误差异常。
- 保持现有实时大屏、今日雷达、午间休息保护和定时任务触发逻辑不变。

### 3. 估值修正诊断能力增强

- 为估值修正链路增加必要的诊断信息，便于排查每一行估值数据的来源。
...
- 将原来可能硬编码在代码中的 AI Provider API Key 和接口地址配置剥离，改为通过 KV 保存和读取。
- 支持在系统设置 Tab 中配置 Google、NVIDIA、OpenRouter、ModelScope 等 Provider 的 API Key 和 API URL。
- 保存系统设置后，AI Provider 配置写入 KV，后续 AI 调用优先从 KV 读取配置。
- 未配置 API Key 的 Provider 不会被错误调用，避免在代码中写死真实密钥，提升配置安全性。
...
- 午间休息期间跳过 `AGENT_SNAPSHOT_V1` 写入，并且不生成今日雷达 Spread 定时点。
...
* **智能阈值对齐 (MarkLine)**：自动提取配置中的 `customBuy`（买点）与 `customSell`（卖点），动态渲染水平虚线。虚线颜色**自动继承**主曲线标识色，大幅提升多临界点并发时的视觉聚合度。
* **前端状态锁 (Legend State)**：完美解决轮询刷新时图表显示/隐藏状态被重置的交互痛点。在 `setOption` 前自动截获并回填 `legend.selected` 状态字典。采用纯前端内存级保持，**无需读写 KV 或本地存储**，刷新网页 (F5) 自动释放。
* **轴线自适应与防遮挡 (Scale)**：引入 `scale: true` 机制使 Y 轴自适应缩放，确保极端阈值线不滑出可视区域。同时将右侧边距拓宽至 `6%` 并优化标签排版，防止提示文字（“买/卖 X%”）被边缘裁切。现在还是不显示买点和卖点
